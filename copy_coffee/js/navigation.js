(function() {
  function initNavigation() {
    // Mobile Menu Toggle
    const menuBtns = document.querySelectorAll('.dashboard-menu-btn, .mobile-menu-btn, button[aria-label="Open menu"], button[aria-label="Open Navigation"]');
    ...
    const closeBtns = document.querySelectorAll('.dashboard-drawer-close, .drawer-close');
    ...
    const backdrops = document.querySelectorAll('.dashboard-drawer-backdrop, .drawer-backdrop');
    ...
    // Cart Drawer Toggle
    const cartBtns = document.querySelectorAll('[data-lucide="shopping-cart"], [aria-label="Cart"]');
    cartBtns.forEach(btn => {
      const parentBtn = btn.closest('button');
      if (parentBtn) {
        parentBtn.addEventListener('click', () => {
          alert("Cart feature is currently a frontend preview.");
        });
      }
    });
    ...
    if (adminTabs.length > 0) {
      adminTabs.forEach(tab => {
        tab.addEventListener('click', () => {
          ...
          adminTabs.forEach(t => {
            t.classList.remove('bg-brand-medium/50', 'text-brand-gold', 'border-r-2', 'border-brand-gold');
            t.classList.add('text-brand-latte/70', 'hover:bg-brand-medium/30');
          });
          tab.classList.add('bg-brand-medium/50', 'text-brand-gold', 'border-r-2', 'border-brand-gold');
          tab.classList.remove('text-brand-latte/70', 'hover:bg-brand-medium/30');
    ...
    const backToLoginBtns = document.querySelectorAll('[id^="auth-back-to-login"]');
